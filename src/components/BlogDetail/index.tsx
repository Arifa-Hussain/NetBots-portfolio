import { Box, Typography,List, ListItem, ListItemIcon, ListItemText, Grid, Card, CardContent, CardMedia  } from "@mui/material";
      ...
      {Object.keys(highlights)
        .filter(
          (key) =>
            ...
            highlights[key as keyof typeof highlights] !== undefined
        )
        .map((key, index) => (
          <ListItem key={index}>
            <ListItemIcon>
            ...
            <ListItemText 
              primary={String(highlights[key as keyof typeof highlights])} 
              sx={{
...
>
  ...
  {recentPosts
    .filter((post) => [3, 4, 5].includes(Number(post.id))) // Convert id to number
    .map((post) => (
      <Box
        key={post.id}
        sx={{
...
<Box sx={{ padding: 2 }}>
  ...
  <Grid item xs={12} md={12} id="section-7">
    ...
    >
      ...
      >
        {careerSection?.heading || "Default Heading"}
      </Typography>
      <List>
        {Object.keys(categoriesSection)
          .filter((key) => key.startsWith("description") && categoriesSection[key as keyof typeof categoriesSection])
          .map((key, index) => (
            <ListItem key={index} disableGutters sx={{ padding: "4px 0" }}>
              ...
              <ListItemText
                primary={categoriesSection[key as keyof typeof categoriesSection] as string}
                primaryTypographyProps={{
                  variant: "body1",
                  sx: { color: "#333", fontSize: { xs: "0.9rem", sm: "1rem" } },
                }}
              />
            </ListItem>
          ))}
...
<Box sx={{ padding: 2, borderRadius: 2 }}>
 <Typography
  ...
  {BlogData.filter((blog) => ['10', '11', '12'].includes(blog.id.toString())).map((blog) => (
          ...
          <Grid container justifyContent="space-between">
            ...
            <Grid item>
              <Typography variant="body2" sx={{ color: "#2A327D" }}>
                {blog.date ? new Date(blog.date).toLocaleDateString() : "Date Not Available"}
              </Typography>
            </Grid>
          </Grid>
            ...
            <Grid item>
              {blog.profile && (
                <CardMedia
                  ...
                  height="40"
                  image={blog.profile}
                  alt={`${blog.name || "Author"}'s profile`}
                  sx={{ borderRadius: '50%' }}
                />
              )}
            </Grid>
